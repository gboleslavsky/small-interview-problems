/**
  ...
  * I am not sure there is a more expressive solution in another language, this one appears pretty transparent and simple
  * and i think shows the good parts of Scala
  */
...
object ValidTime {
  ...
  def validTime(a: Int, b: Int, c: Int, d: Int): String = {
    val possible =          List(a, b, c, d).permutations.map(p => p.mkString).toSet
    val minutes =           List.range(0, 60).map(n => f"$n%02d").toSet
    val hours =             List.range(0, 24).map(n => f"$n%02d").toSet
    val valid =             (for (h <- hours; m <- minutes) yield h + m)
    val possibleAndValid =  valid.intersect(possible)

    if (possibleAndValid.isEmpty)
      INVALID_TIME
    else
      possibleAndValid.max
  }
